<script>
var objects_import_v2 = {
	"data": function(){
		return {
			"token": "",
			"vshow": true,
			"msg": "", "err": "", "msg2": "", "err2": "", "msg3": "", "file_err": false, 
	...
	props:[ "refname", "path", "vtab", "app_id" ],
	mounted: function(){
	...
	"methods": {
		check_template: function(){
			try{
				...
				for( var fd in this.vtab['vimport']['template'] ){
					s[ "props" ][ fd ] = {"type": "text", "name": this.vtab['vimport']['template'][ fd ]['l']['v']+'', "map": -1, "csvf": "", "targetf": "", "use": true, "m": true, "primary": false, "label": false};
				}
				...
				console.error( e );
				this.import_err = "Error Checkng Template: " + e;
		...
		token_validate(t){
			if( t.match(/^(SessionChanged|NetworkChanged)$/) ){
				this.import_err = "Login Again";
				alert("Need to Login Again");
			}else{
				this.import_err = "Token Error: " + t;
			}
		},
		get_size: function(){
			var s = this.vf.size;
		...
		checkfile: function(){
			...
			if( this.upload_type == "CSV" ){
				...
				if( fields.length < 2 ){
					...
					this.import_err = "File is not in CSV Format";
					this.file_err = true;
					console.log( d );
					this.sample_data = d;
				}else{
		...
		checkfile_json: function(){
			...
			if( this.filedata.substr(0,1) == "[" && this.filedata.substr( this.filedata.length-1, 1) == "]" ){
				...
				for(var i=0;i<20;i++){if( this.fpos < this.filedata.length-1 ){
					...
					if( ipos == -1 ){
						this.import_err = "File end may not reached";
						break;
					}else{
		...
		checkfile_csv: function(){
			...
			if( this.vf.size > 1024*1024*5 ){
				this.import_msg = "File size is more than 5 MB";
				this.file_err = true;
			}
			...
			for(var i=0;i<100;i++){
				...
				if( typeof(d) == "object" ){
					recs.push(d);
					this.tot_cnt++;
				}else if( d == "end" ){ break; }else{ this.import_err = d; this.file_err = true; break; }
			...
			this.sample_records = recs;
			this.step = 2;
			if( r_cnt != c_cnt ){ this.import_err = "Header and records column count not same.";this.file_err = true; }
			if( issue_cnt > 10 ){
				this.import_err = this.import_err  + " Column count inconsistent in " + issue_cnt + "% records";this.file_err = true;
			}else if( issue_cnt > 0 ){
				this.import_err = this.import_err + " Column count issues";
			}
		...
		checkfile_csv_continue: function(){
			while( 1 ){
				...
				else{ this.analyzing = false; this.import_err = d; this.file_err = true; break; }
			}
		},
		openbrowse: function(){
		...
		dataset_check: function(){
			setTimeout(this.dataset_check2,200);
		},
		dataset_check2: function(){
			//this.import_primary_field = "";
			this.import_label_field = "";
			this.import_alias_field = [];
			if( this.vtab['vimport']['i_of']['v'] != "" ){
		...
		doimport2: function(){
			this.import_err = "";
			if( this.vtab['vimport']['i_of']['v'] == "" ){
				this.import_err = "Need Instance Node"; return false;
			}
			if( this.upload_type == "CSV" ){
				if( this.dataset == false ){
					if( this.import_primary_field.trim() == "" ){
						this.import_err = "Need ID (primary key) field"; return false;
					}else if( this.import_label_field.trim() == "" ){
						this.import_err = "Need Label Unique field"; return false;
					}
				}
				for( var fd in this.sch_keys ){if( this.sch_keys[fd]['use'] ){
					if( fd != this.import_primary_field && fd != this.import_label_field ){
						if( this.sch_keys[ fd ]['type'] == "" ){
							this.import_err = "Field type required for `"+fd+"` "; return false;
						}
						if( this.sch_keys[ fd ]['targetf'] == "" ){
							this.import_err = "Field mapping required for `"+fd+"` "; return false;
						}
					}
					if( fd != this.import_primary_field ){
						if( this.sch_keys[ fd ]['type'] == "graph-thing" ){
							if( this.sch_keys[ fd ]['i_of']['v'] == "" ){
								this.import_err = "Field `"+fd+"` linkable instance name required"; return false;
							}
						}
					}
				}}
							...
							if( d.length > prime_index ){
								if( d[prime_index] in prime_keys == false ){
									...
									if( d[prime_index].match(/^[a-z0-9]{5,24}$/) == null ){
										this.import_err = "Primary field value `" + d[prime_index] + "` format is not allowed for record: "+rcnt; return false;
									}
								}else{
									prime_keys[ d[prime_index] ]++;
									this.import_err = "Primary field value `" + d[prime_index] + "` repeated for record: "+rcnt; return false;
								}
							}
						}
						if( this.dataset == false ){
							if( label_index != -1 ){
								if( d.length > label_index ){
									if( d[label_index] in label_keys == false ){
										...
										this.import_err = "Label value `" + d[label_index] + "` repeated for record: "+rcnt; return false;
									}
								}
							}
						}
					}else if( d == "end" ){ break; }else{ this.import_err = "Failed to read file";return false; break; }
				}
			}
		...
		start_importing: function(){
			...
			this.import_err = "";
			this.import_msg = "Initiating...";
			this.fpos =0;
			axios.post("?", {
				"action":"objects_import_data_check",
				"dataset": this.dataset,
				"object_id": this.vtab['vimport']['i_of']['i'],
				"schema": this.sch_keys, "primary": this.import_primary_field, "label": this.import_label_field,
				"dataset": this.dataset,
					...
					if( typeof(response.data) == "object" ){
						if( 'status' in response.data ){
							if( response.data['status'] == "success" ){
								...
								this.import_err = "Token Error: " + response.data['data'];
							}
						}else{
							this.import_err = "Incorrect response";
						}
					}else{
						this.import_err = "Incorrect response Type";
					}
				}else{
					this.import_err = "Response Error: " . response.status;
				}
			});
		},
		...
		start_importing_csv_batch: function(){
			...
			for(var i=0;i<this.csv_batch_limit;i++){
				...
				if( typeof(d) == "object" ){
					...
					recs.push( rec );
				}else if( d == "end" ){ break; }else{ this.import_err = d; break; }
			}
			if( recs.length ){
						...
						if( typeof(response.data) == "object" ){
							if( 'status' in response.data ){
								if( response.data['status'] == "success" ){
									...
									this.import_err = "Import Error: " + response.data['error'];
								}
							}else{
								this.import_err = "Incorrect response";
							}
				...
				if( this.fpos < this.filedata.length-1 ){
					...
					if( ipos == -1 ){
						...
						try{var rec = JSON.parse(j);}
							...
							this.import_err = "File json parse failed: " + e;
							return;
						}
						recs.push(rec);
						this.fpos=ipos;
		...
		template_edit_popup_open: function(){
			this.$root.import_template_edit_popup_open( this.vtab['vimport']['i_of']['i'], this.vtab['vimport']['i_of']['v'] );
		},
			...
			axios.post("?", {
				...
				if( typeof( response.data['data'] )=="object" ){
					if( this.dataset == true ){
						if( response.data['data']['i_t']['v'] != "L" ){
							this.template_error = true;
							this.import_err = "Selected Instance is not a DataSet";
							return false;
						}
					}else{
						if( response.data['data']['i_t']['v'] != "N" ){
							this.template_error = true;
							this.import_err = "Selected Instance is not a Node Template";
							return false;
						}
					}
					if( "z_t" in response.data['data'] == false ){
						this.template_error = true;
						this.import_err = "Template is not defined for " +  this.vtab['vimport']['i_of']['v'];
						return false;
					}
					this.vtab['vimport']['template'] = response.data['data']['z_t'];
		...
		new_thing_created: function(vdata){
			this.vtab['vimport']['i_of']['i'] = vdata['object_id'];
			this.vtab['vimport']['i_of']['v'] = vdata['label'];
			this.import_select_iof();
		}
	},
	template: `<div>
			...
			<div v-if="step==2" >
				<div style="height:50px; " >
					...
					<input v-else-if="tot_cnt<=20000&&file_err==false" type="button" class="btn btn-outline-dark btn-sm" v-on:click="doimport" value="Next" style="float:right; margin-right: 10px;">
				...
				<div style="overflow: auto;max-height: calc( 100% - 130px - 20px ); padding-right:10px;">
			...
			<div v-if="step==3" >
				<div>
					<input v-if="tot_cnt<=20000&&template_error==false" type="button" class="btn btn-outline-dark btn-sm" v-on:click="doimport2" value="Next" style="float:right; margin-right: 10px;">
						...
						<div>
							...
							<div>
								<div class="code_line codeline_thing" >
									<div title="Thing" data-type="dropdown" v-bind:data-var="'ref:'+refname+':vtab:vimport:i_of:v'" data-list="graph-thing" v-bind:data-thing="'GT-ALL'" data-thing-label="Things" v-bind:data-context-callback="refname+':import_select_iof'" >{{ vtab['vimport']['i_of']['v'] }}</div>
									<div v-if="vtab['vimport']['i_of']['v']&&vtab['vimport']['i_of']['i']" class="btn btn-link btn-sm" v-on:click="template_edit_popup_open()" >Edit Template</div>
									<div v-if="vtab['vimport']['i_of']['v']&&vtab['vimport']['i_of']['i']" class="btn btn-link btn-sm py-0" v-on:click="import_select_iof()" ><i class="fa-solid fa-arrows-rotate"></i></div>
								</div>
							</div>
						</div>
						<div>
					...
					<p><label style="cursor:pointer;" ><input type="checkbox" v-model="dataset" v-on:click="dataset_check()" > Upload as a dataset without any unique label/primary key</label></p>

				</div>
				<div style="overflow: auto;max-height: calc( 100% - 200px ); ">

					<template v-if="upload_type=='CSV'" >
						<template v-if="vtab['vimport']['i_of']['v']==''" >
							...
							<table class="table table-bordered table-sm w-auto">
								<thead>
									<tr class="text-bg-light">
										...
										<td v-if="dataset==false" align="center" width="50">Label</td>
										<td v-if="dataset==false" align="center" width="50">Alias</td>
										<td>Import</td>
								...
								<tbody>
									<tr v-for="fd,f in sch_keys" >
										...
										<td v-if="dataset==false" align="center"><input v-if="import_primary_field!=f&&f!='_default_id'" type="radio" v-model="import_label_field" v-bind:value="f" v-on:click="import_label_check(f)" ></td>
										<td v-if="dataset==false" align="center"><input v-if="import_label_field!=f&&f!='_default_id'" type="checkbox" v-model="import_alias_field" v-bind:value="f" v-on:click="import_alias_check(f)" ></td>
			...
			<div v-if="step==4" >
				...
				<div style="height:30px;border:1px solid #6455af;">
					<div style="position:absolute; padding:0px 10px;" >Progress <span style="font-size:1.2rem;">{{ upload_progress }} %</span> </div>
					<div v-bind:style="'height:30px;background-color:#a18eff; width:'+(upload_progress)+'%;overflow:hidden;'" >&nbsp;</div>
				</div>
			...
			<div style="height: 30px; padding-right:10px;" >
				<div v-if="import_msg" style="color:blue;float:left;" >{{ import_msg }}</div>
				<div v-if="import_err" style="float:right;color:red;float:right;" >{{ import_err }}</div>
			</div>
</div>`
};
</script>