...
<?php
	$css_data = file_get_contents("page_apps_pages_page_html_editor_css.css");
	//$template_data = file_get_contents("page_apps_pages_page_html_editor_template.html");
	...
	require("page_apps_pages_page_html_tag_config.js");
...
var app = s2_ssssssssss({
	data(){
		return {
			...
			s2_eeeeeeegap: <?=json_encode($page) ?>,
	...
	mounted(){
		...
		document.addEventListener("mousedown", this.s2_nwodesuom_tneve);
		document.addEventListener("mouseup", this.s2_puesuom_tneve);
		document.addEventListener("mousemove", this.s2_evomesuom_tneve);
		this.frame = this.$refs.s2_emarfi_rotide.contentWindow;
		this.control_frame = this.$refs.s2_emarfi_lortnoc;
		
		const new_style_element = document.createElement("style");
		new_style_element.id = "s2_ssc_pot_rotide";
    	...
    	var css_link = document.createElement("link");
    	css_link.href=this.rootpath+'page_apps_pages_page_html_editor_body_css.css';
    	css_link.setAttribute("rel", "stylesheet");
    	this.frame.document.head.appendChild(css_link);
	...
	methods: {
		...
		set_preview_urls: function(){
			...
			var urls = {};
			var ulist = [];
			if( 'cloud' in this.s2_pppppppppa['settings'] ){if( this.s2_pppppppppa['settings']['cloud'] ){
				var u = "https://" + this.s2_pppppppppa['settings']['cloud-subdomain'] + '.' + this.s2_pppppppppa['settings']['cloud-domain'] + '/' + this.s2_pppppppppa['settings']['cloud-enginepath'] + this.s2_eeeeeeegap['name'];
				urls['cloud'] = u;
				ulist.push( u );
				if( 'alias' in this.s2_pppppppppa['settings'] ){if( this.s2_pppppppppa['settings']['alias'] ){
					var u = "https://" + this.s2_pppppppppa['settings']['alias-domain'] + this.s2_eeeeeeegap['name'];
					urls['alias'] = u;
					ulist.push( u );
				}}
			}}
			console.log("OKK2");
			if( 'domains' in this.s2_pppppppppa['settings'] ){
				urls['domains'] = [];
				for(var d=0;d<this.s2_pppppppppa['settings']['domains'].length;d++ ){
					var u = this.s2_pppppppppa['settings']['domains'][ d ]['url'] + this.s2_eeeeeeegap['name'];
					urls['domains'].push( u );
					ulist.push( u );
				}
			}
			...
			this.s2_ooooooohce( this.vurls );this.s2_ooooooohce( this.vurls_list );
		},
		previewit: function(){	
		...
		switch_tab: function(v){
			...
			if( this.edit_tab == 'source' ){
				if( this.ace_editor2 ){
					this.s2_eeeeeeegap['html'] = this.ace_editor2.getValue();
					this.html2 = this.s2_eeeeeeegap['html'];
				}
			}
			if( this.edit_tab == 'html' ){
				this.s2_eeeeeeegap['html'] = this.frame.document.getElementById(this.target_editor_id).innerHTML;
				this.html2 = this.s2_eeeeeeegap['html'];
			}
			if( this.edit_tab == 'scriptsource' ){
				this.s2_eeeeeeegap['script'] = this.ace_editor3.getValue();
			}
			if( v == "source" ){
				this.init_ace2();
			}
			...
			if( v == "html" ){
				this.frame.document.getElementById(this.target_editor_id).innerHTML = this.s2_eeeeeeegap['html'];
			}
			this.edit_tab = v;
		},
		init_ace2: function(){
			...
			this.ace_editor2.setValue( html_beautify(this.s2_eeeeeeegap['html']) );
		},
		init_ace3: function(){
			console.log("Ace initialized");
			...
			this.ace_editor3.setValue( js_beautify(this.s2_eeeeeeegap['script']) );
		},
		...
		save_page2(){
			...
			if( 'settings' in this.s2_eeeeeeegap == false ){
				this.s2_eeeeeeegap['settings'] = {"one":1};
			}
			this.s2_ggggggggsm = "Saving.......";
			this.s2_rrrrrrrrre = "";
			axios.post("?",{
				...
				"html": this.s2_eeeeeeegap['html'],
				"script": this.s2_eeeeeeegap['script'],
				"settings": this.s2_eeeeeeegap['settings'],
			}).then(response=>{
				this.s2_ggggggggsm = "";
				if( response.status == 200 ){
					if( typeof(response.data) == "object" ){
						if( 'status' in response.data ){
							if( response.data['status'] == "success" ){
								...
								this.s2_rrrrrrrrre = "Error: " + response.data['error'];
							}
						}else{
							this.s2_rrrrrrrrre = "Incorrect response";
						}
		...
		s2_puesuom_tneve: function(e){
			this.md = false;
		},
		s2_nwodesuom_tneve: function(e){
			this.md = true;
		},
		s2_evomesuom_tneve: function(e){
			if( this.md ){
			...
			keydown_iframe: function(e){
				this.s2_ooooooohce("keydowniframe"+e.keyCode);
			...
			keydown: function(e){if( this.enabled ){
				this.s2_ooooooohce("keydown: "+e.keyCode);
				if( e.keyCode == 27 ){ // escape
					//this.insert_tag =false;
					this.hide_other_menus();
					return false;
			...
			s2_semaNssalc_desucof_tes: function(){
			...
			set_focused2: function( v, vfromkeydown=false ){
				...
				}
				{
					...
					//this.s2_semaNssalc_desucof_tes();
					// = v.className;
			...
			find_sections: function(){
				this.s2_ooooooohce("find_sections: ");
				this.focused_li = false;
				var sel = this.frame.document.getSelection();
				if( sel.rangeCount ){
					var sr = this.frame.document.getSelection().getRangeAt(0);
					...
					if( sr.commonAncestorContainer.nodeName.match(/^(UL|OL)$/) && sr.commonAncestorContainer.nodeName == sr.startContainer.nodeName && sr.commonAncestorContainer.nodeName == sr.endContainer.nodeName && sr.startContainer == sr.endContainer ){
						...
						if( vids.length ){
							...
							this.frame.document.getSelection().removeAllRanges();
							this.frame.document.getSelection().addRange(r);
							this.s2_ooooooohce( "found sections:" );
							this.s2_ooooooohce( vids );
							return vids;
						}
						return false;
					}
			...
			create_new: function(v){
				...
				this.s2_ooooooohce( newel.innerHTML );
