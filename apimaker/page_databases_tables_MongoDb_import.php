			...
			<div style="overflow: auto;height: calc( 100% - 130px - 50px - 30px ); padding-right:10px;">
				...
				<div v-if="step==3" >
					<template v-if="upload_type=='CSV'" >
						...
						<p v-if="msg2" v-html="msg2" ></p>
						<p v-if="err2" style="color:red;" >{{ err2 }}</p>
							...
							<tbody>
								<tr v-for="fd,f in sch_keys" >
									...
									<td>
										<select class="form-select form-select-sm w-auto" v-model="sch_keys[ f ]['map']" v-on:change="import2_check" >
											<option value="-1" >Not Mapped</option>
											<option v-for="hidx,hd in fields_match" v-bind:value="hd" >{{ hd }}</option>
										</select>
...
var app = Vue.createApp({
	...
	methods: {
		...
		import2_check: function(){
			setTimeout(this.import3_check,200);
		},
		import3_check: function(){
			this.err2 = "";
			this.msg2 = "";
			var cnt = 0;
			var matched = {};
			...
			//this.echo__(matched);
			//if( cnt < (Object.keys(this.sch_keys).length/2) )
			{
		...
		doimport2: function(){
			...
			this.echo__(matched);
			if( cnt == 0 ){
				this.err2 = "at lease one key mapping is required for import";return false;
			}else if( cnt <= (Object.keys(this.sch_keys).length/2) ){
				this.err2 = "You have not mapped at least 50% of fields ";
				return false;
			}
