		...
		<div style="padding: 10px;" >
			...
			<div style="position:relative;overflow: auto; height: calc( 100% - 60px );">
				...
				<template v-if="enabled==false" >
					<p><b>Objects are enabled</b></p>
					<p>You may have to goto configuration and initialize database</p>
					<div>
						<div class="btn btn-sm btn-outline-dark" v-on:click="show_configure()" >Configure</div>
					</div>
				</template>
				<template v-else >
					<div v-if="thing_id!=-1" >
		...
		<div class="modal-content">
		  <div class="modal-header">
			...
			<div class="modal-body" >
				<template v-if="enabled==false" >
					<p>Objects database is not enabled</p>
					<p>Click here to enable and initialize the database</p>
					<p><input type="button" value="Enable" class="btn btn-outline-dark btn-sm" v-on:click="enable_objects" ></p>
				</template>
				<template v-else>
					<p>Objects database is enabled</p>
					<p>Click here to Disable and delete the database</p>
					<p><input type="button" value="Disable" class="btn btn-outline-dark btn-sm" v-on:click="disable_objects" ></p>
				</template>
		  	</div>
		</div>
	</div>
				...
				if( this.context_menu_key__ == "" ){
					...
					for(var i=0;i<v2.length&&v3.length<100;i++){
						...
						if( v2[i]['l']['v'].match(kpr) ){
							...
							if( 'ol' in vres ){
								if( vres['ol'] != vres['l']['v'] ){
									vres['r'] = "<b>" + vres['r'] + "</b> alias <b>"+vres['ol']+"</b> in <span class='text-secondary'>" + vres['i_of']['v'] + "</span> ["+ vres['i'] + "]";
								}else{
