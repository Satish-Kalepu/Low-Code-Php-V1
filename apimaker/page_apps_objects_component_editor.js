var editor_component = {
	...
	mounted: function(){
		...
		if( editor_component['css_template__'] ){
			...
			if( document.getElementById("editor_css_style_tag__") == null ){
				var vl = document.createElement("style");
		...
		set_focus_to__: function( vi ){
			...
			contextmenu_set_style__: function(){
				...
				try{
					...
					console.log( this.clientX__ + "xx" + this.clientY__ );
				}catch(e){
				...
				this.contextmenu__ = true;
				console.log( this.contextmenu_style__ );
			},
			contextsidemenu_set_style__: function(el){
				...
				this.contextsidemenu__ = true;
				console.log( this.contextsidemenu_style__ );
			},
			...
			initialize_tables__: function(){
				var vtables__ = document.getElementById(this.target_editor_id__).getElementsByTagName("table");
			...
			make_ol__: function(){
				...
				if( this.sections_list__.length == 0 ){
					if( this.focused__.nodeName.match(/^(P|DIV)$/) ){
						...
						console.log( this.focused__.childNodes[ 0 ].nodeName );
			...
			make_ul__: function(){
				this.hide_bounds__();this.hide_contextmenu__();
				console.log( this.sections_list__ );
				if( this.sections_list__.length == 0 ){
					if( this.focused__.nodeName.match(/^(P|DIV)$/) ){
						this.sections_list__ = [this.focused__];
					}else if( this.focused__.nodeName.match( /^(TD|TH)$/ ) ){
			...
			remove_bullets__: function(){
				...
				if( vs.previousElementSibling == null && ve.nextElementSibling == null ){
					...
					var newul__ = document.createElement("UL");
			...
			set_focused2__: function( v, vfromkeydown__=false ){
				...
				if( v.hasAttribute("data-id") ){
					if( v.getAttribute("data-id") == "root" ){
						...
						//return ;						
						var v = document.getElementById("editor_div");
			...
			table_insert_row__: function(vt){
				...
				var vtr = this.focused_td__.parentNode;
				var vl = document.createElement("TR");
				for(var i=0;i<vtr.children.length;i++){
					var vl2 = document.createElement("TD");
					vl.appendChild(vl2);
				}
			...
			convert_tag_to__: function(vto){
				...
				if( this.focused_type__.match(/^(P|DIV|H1|H2|H3|H4)$/) ){
					if( vto.match(/^(P|DIV|H1|H2|H3|H4)$/) ){
						var vl = document.createElement(vto);
						vl.innerHTML = this.focused__.innerHTML;
...
var css_template__ = `
.contextmenu .tag_btn{ display:inline-block; padding:5px; border:1px solid #ccc; background-color:white; cursor:pointer; margin-left:2px; }
.contextmenu .tag_btn_a{cursor:initial; pointer-events:none;}
.contextmenu .tag_btn:hover{ background-color:#f0f0f0; }
...
body #editor_div{ outline:none; }

#editor_div select,#editor_div input, #editor_div textarea, #editor_div button{outline: 0px;}
#editor_div img{ min-width:50px; min-height:30px; cursor:pointer; }
#editor_div img[src=""]{ background-color:#ccc; }

.side_dragger__{ position:absolute; background-color:rgba(255,255,255,0.5); user-select:none; z-index:510; cursor:pointer; }

.editor_popup{ position: absolute; border: 1px solid #bbb; box-shadow: 1px 2px 2px #666; z-index: 500; background-color:white; }
.editor_popup2{ position: absolute; border: 1px solid #333; box-shadow: 1px 2px 2px #666; z-index: 500; padding: 10px; background-color:white; }

.focused_bounds{ position:absolute; z-index:401; top:100px; left:100px; width:300px; height:300px;outline:3px dashed rgba(155,155,155); pointer-events:none; }
.focused_bounds_tip{ position:absolute; z-index:401; right:0px; top:-20px; height:20px; background-color: rgb(155,155,155,0.5); cursor: pointer; min-width:20px; text-align: center; color: white; font-weight: bold; font-size:10px; padding: 0px 5px; line-height:20px; overflow:hidden; }

.table_cell__{ position:absolute; z-index:401; outline:2px solid rgba(0,0,200,0.5); pointer-events:none; }

.side_settings_bar{ position:absolute; z-index:402; top:100px; left:100px; height:5px; background-color:rgb(255,55,55,0.2); cursor:pointer; }
.side_settings_bar:hover{}
.side_settings_bar_sub{ width:100%; height:10px; margin-top:-3px; text-align:center; cursor:pointer; display:none; }
.side_settings_bar_sub2{ width:100%; height:5px; margin-top:2px; text-align:center; cursor:pointer; background-color:rgb(255,55,55,0.2); }
.side_settings_bar:hover .side_settings_bar_sub{ height:10px; background-color:rgba(255,5,5,0.2); display:block; }
.side_settings_bar_sub svg{color:orange !important; margin-top:-5px}

.drop_target_bar{ position:absolute; z-index:402; top:100px; left:100px; height:5px; background-color:rgb(255,255,255,0.5); cursor:pointer; }

.link_suggest{ position: absolute; padding: 2px; min-width: 300px; max-width: 600px; max-height: 300px; overflow: auto; z-index: 501; background-color: white; box-shadow: 2px 2px 4px #666; border: 1px solid #333; }
.link_suggest div {border-bottom: 1px solid #bbb; padding: 2px; cursor: pointer; white-space: nowrap;}
.link_suggest div.link_suggest_select {background-color: #ffcefe;}
.link_suggest div:hover {background-color: #ffcefe;}
.link_suggest div span span{color: red;}
.link_suggest div span:nth-child(1) {font-weight: 600;}
.link_suggest div span:nth-child(2) {font-style: italic; font-weight: bold; color: #aaa;}
.link_suggest div span:nth-child(3) {color: maroon;}
.image_inline_menu{ position: absolute; z-index: 404; background-color: white; box-shadow: 3px 3px 5px black; padding: 10px; }
.image_inline_menu a{ text-decoration: none; }

.image_popup{ position: fixed; min-width: 800px; max-width: 90%; top: 50px; left: 50px; height: calc( 100% - 100px ); z-index: 404; background-color: white; border: 1px solid #666; box-shadow: 2px 2px 5px black; border-radius:5px; }
.image_popup_back{ position: fixed; width:100%; height: 100%; background-color: rgba(0,0,0,0.5); top: 0px; left: 0px; z-index: 403; }
.image_popup_head{ border-bottom: 1px solid #333; padding: 10px 20px; border-top-left-radius:5px;border-top-right-radius:5px; }
.image_popup_head span{ font-weight: bold; }
.image_popup_cls_btn {float: right; cursor: pointer; border-top: 1px solid #ccc;}
.image_popup_cls_btn:hover{ color:red; }
.image_popup_content{ height: calc( 100% - 35px); overflow: auto; padding: 10px; border-radius:5px; }
.image_popup_tab{ display: inline-block; padding: 0px 5px; margin-right:5px; border: 1px solid #ccc; cursor: pointer; border-bottom:5px solid #ccc; }
.image_popup_tab:hover{ background-color: #ffe9d1; }
.image_popup_tab.active{ border-bottom:5px solid orange; }

.image_crop_popup{ position: fixed; min-width: 500px; max-width: 90%; top: 50px; left: 50px; height: calc( 100% - 100px ); z-index: 404; background-color: white; border: 1px solid #666; box-shadow: 2px 2px 5px black; border-radius:5px; }


.pre_edit_popup__{ position: absolute; background-color: white; border: 2px solid #e0e0e0; padding: 10px; box-shadow: 3px 3px 3px #666; z-index: 500; }
.pre_edit_popup__ textarea{ height: calc( 100% - 50px ); }

.attachment_popup{ position: fixed; top: 200px; left:200px; border-radius: 1px solid #ccc; box-shadow: 2px 2px 5px black; z-index: 500; background-color: white; }

.contextmenu{ position: absolute; background-color: white; border: 1px solid #999; box-shadow: 2px 2px 5px #666; z-index: 500; white-space: nowrap; }
.contextmenu_btn { cursor: pointer; border:1px solid #aaa; }
.contextmenu_btn:hover { background-color: #ccc; }
.contextmenu_submenu__{ position: absolute; background-color: white;border: 1px solid #999; box-shadow: 2px 2px 5px #666; padding: 5px; z-index: 402;  }
.contextsidemenu__{ position: absolute; background-color: white;border: 1px solid #999; box-shadow: 2px 2px 5px #666; padding: 5px; z-index: 501; }
.contextmenu a{ background-color: #f0f0f0; text-decoration: none; border:1px solid #aaa; padding:0px 3px; }
.contextmenu a:hover{ background-color: #eee; border:1px solid #999; }

#editor_div ul{ padding-bottom: 10px; }
#editor_div a:empty:before {content: '       ';white-space: pre;}
#editor_div p:empty:before {content: ' ';white-space: pre;}
#editor_div div:empty:before {content: ' ';white-space: pre;}
#editor_div span:empty:before {content: ' ';white-space: pre;}
#editor_div h1:empty:before {content: ' ';white-space: pre;}
#editor_div h2:empty:before {content: ' ';white-space: pre;}
#editor_div h3:empty:before {content: ' ';white-space: pre;}
#editor_div h4:empty:before {content: ' ';white-space: pre;}
#editor_div pre:empty:before {content: ' ';white-space: pre;}
#editor_div li:empty:before {content: ' ';white-space: pre;}
#editor_div td:empty:before {content: ' ';white-space: pre;}
#editor_div th:empty:before {content: ' ';white-space: pre;}
`;
var editor_template__ = `<div>
	<div id="editor_controlls" >
			...
			<template v-if="contextmenu_submenu__" >
				...
				<div style="padding: 5px; height: 200px; overflow: auto;">
					<div v-if="contextmenu_submenu_type__=='A'" style="width:350px;resize:both;" >
						...
						<div>URL:</div>
						<div><input type="text" class="form-control form-control-sm" v-model="anchor_href__" placeholder="URL" ></div>
						<div>&nbsp;</div>
