...
if( $_POST['action'] == "objects_load_browse_list" ){
	...
	$res = $mongodb_con->find( $graph_things, $cond, [
		'projection'=>['l'=>1, 'i_t'=>1, 'i_of'=>1, 'm_i'=>1, 'm_u'=>1,'cnt'=>1],
		'sort'=>$sort,
		'limit'=>100,
	]);
	$res['cnt'] = $cnt;
...
if( $_POST['action'] == "objects_edit_type" ){
	...
	if( $current_type == "L" && $type['v'] != "L" ){
		if( isset($object['cnt']) && $object['cnt'] > 0 ){
			json_response("fail", "There are records " . $object['cnt'] . " under this DataSet");
		}
	}
...
if( $_POST['action'] == "objects_dataset_record_create" ){
	...
	$res2 = $mongodb_con->increment( $graph_things, $thing_id, "cnt", 1 );
...
if( $_POST['action'] == "objects_create_with_template" ){
	...
	if( $thing['i_t']['t'] == "L" ){
		if( !isset($thing['z_t']) ){
			json_response("fail", "Template is must for a DataSet");
		}
	}
	if( $thing['i_t']['t'] == "D" || $thing['i_t']['t'] == "M" ){
		if( isset($thing['z_t']) ){
			unset($thing['z_t']);unset($thing['z_o']);unset($thing['z_n']);
		}
	}
...
if( $_POST['action'] == "objects_create_node_on_fly" ){
	...
	$instance = $res['data'];
	if( $instance['i_t']['v'] != "N" ){
		json_response("fail", "Instance is not of type Node. Sub Nodes are not allowed");
	}
